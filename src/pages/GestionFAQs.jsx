...
// Esta lista es para el <Form.Select> de TIPO_DATO.
// Asegúrate que los 'value' coincidan con los 'nombre' en tu tabla `contacto_tipos_dato`
...
const GestionContactanos = () => {
  ...
  // No necesitamos cargar las categorías de contacto para el input, pero sí para la tabla
  // y si el backend las crea, fetchData las refrescará.
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const { categoria, tipo_dato, etiqueta, valor, orden, activo } = formData; // activo se obtiene de formData
    ...
    const payload = {
        ...
        tipo_dato: String(tipo_dato).trim(), // El valor del select ya es el nombre/slug
        etiqueta: String(etiqueta).trim(),
        valor: String(valor).trim(),
        icono: formData.icono || null, // Asegurar que se envíe null si está vacío
        orden: numericOrden,
        activo: !!activo // Asegurar que sea booleano para el backend
    };
  ...
  const handleDeleteItem = async (id, etiquetaItem) => {
    // ... (sin cambios, pero asegurarse que usa getAuthHeaders())
    if (!token) { /* ... */ return; }
    Swal.fire({ /* ... */ }).then(async (result) => {
      if (result.isConfirmed) {
        try {
          // ... (Swal.showLoading) ...
          await axios.delete(`${API_URL_ADMIN}/${id}`, getAuthHeaders()); // Asegurar getAuthHeaders
          // ... (Swal.fire success) ...
          fetchContactos();
        } catch (error) {
          // ... (Swal.fire error) ...
